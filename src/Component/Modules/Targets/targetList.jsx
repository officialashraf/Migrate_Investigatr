...
import TargetCreate from "./targetCreate.jsx";
import TargetUpdate from "./targetUpdate.jsx";
import TargetDetails from "./targetDetails.jsx";
...
const TargetList = () => {
  ...
  const fetchTargets = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${window.runtimeConfig.VITE_APP_API_CASE_MAN}/api/case-man/v1/target`, {
        headers: {
  ...
  const deleteTarget = async (id, name) => {
    ...
    try {
      ...
      const response = await axios.delete(`${window.runtimeConfig.VITE_APP_API_CASE_MAN}/api/case-man/v1/target/${id}`,
        {
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
  ...
  return (
    <>{data &&
      ...
      <div className="resourcesContainer" style={{ border: 'none' }}>
        ...
        <AppButton onClick={togglePopup}> + Add New Target</AppButton>
      </div>
    )
    }
