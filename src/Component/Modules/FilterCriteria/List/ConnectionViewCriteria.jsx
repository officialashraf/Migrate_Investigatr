...
import styles from '../../ConnectionGraph/ConnectionView.module.css';
...
const ConnectionViewCriteria = ({ chipsHeight }) => {
    ...
    const handleAddToSearchCriteria = () => {
        ...
        if (selectedNode.type === 'hashtag') {
            ...
            field = 'serverip';
            displayType = 'IP Address';
        } else if (selectedNode.type === 'subscriber') {
            field = 'mobilenumber';
            displayType = 'Mobile Number';
        } else if (selectedNode.type === 'profile') {
            field = 'socialmedia_from_screenname'; // All profiles go to this field
            displayType = 'Social Media User';
        } else {
        ...
        // Update Redux state for search criteria
        ...
        toast.success(`${displayType} added to Search Criteria!`);
        handleCloseContextMenu();
    };
    useEffect(() => {
    ...
    // Cleanup effect to reset pagination when component unmounts
    useEffect(() => {
        return () => {
            dispatch(setPage(1));
        };
    ...
    useEffect(() => {
        if (Object.keys(queryPayload).length === 0 || !selected) {
            setIsLoading(false);
            setGraphData({ nodes: [], links: [] });
        ...
        const hasFilters = Object.values(queryPayload || {}).some(
            value =>
        ...
        const fetchData = async () => {
            ...
            let payload = {
                ...queryPayload,
                start_time: queryPayload?.start_time || DEFAULT_START,
                end_time: queryPayload?.end_time || DEFAULT_END,
            ...
            // remove empty arrays before sending to API
            ...
            try {
                ...
                const finalGraphData = buildFunction(transformFunction(rawData));
    ...
    useEffect(() => {
        ...
        autoFitDone.current = false; // Reset autofit on new data
        ...
        const node = g.append('g')
            ...
            .join('g')
            // ✅ IMPROVED: Stable drag behavior - other nodes don't move
            .call(drag()
                .on('start', (event, d) => {
                    if (!event.active) simulation.alphaTarget(0.1).restart();
                    // Fix ALL nodes except the dragged one
                ...
                .on('end', (event, d) => {
                    ...
                    // Keep all nodes fixed at their positions
                    graphData.nodes.forEach(node => {
                        node.fx = node.x;
                        node.fy = node.y;
                    });
        ...
        node.on('mouseover', (event, d) => {
            ...
            if (d.type === 'profile') {
                htmlContent = `<b>Profile:</b> ${d.label}<br/><b>Platform:</b> ${d.platform}`;
            } else if (d.type === 'hashtag') {
                htmlContent = `<b>Hashtag:</b> ${d.label}`;
            } else if (d.type === 'subscriber') {
                htmlContent = `<b>Mobile No:</b> ${d.label}`;
            } else if (d.type === 'ip') {
                htmlContent = `<b>IP Address:</b> ${d.label}`;
            } else {
                htmlContent = `<b>Node:</b> ${d.label}`;
            }
        ...
        simulation.on('end', () => {
            ...
            // Fix all nodes in position after simulation ends
            graphData.nodes.forEach(node => {
                node.fx = node.x;
                node.fy = node.y;
            });
    ...
    const renderEmptyMessageText = () => {
        ...
        if (!hasFilters) {
            ...
            message = "No IP connections found for the current criteria";
        }
        else if (selected === "hashtags") {
            message = "No hashtag connections found for the current criteria";
        }
        else {
            message = "Please select a connection field (OSINT or IPDR) to view the graph";
        }
    ...
    if (graphData.nodes.length === 0 || !selected) {
        return (
            <div ref={fullscreenContainerRef} className={styles.containerView} style={{ height: `calc(75vh - ${chipsHeight || 0}px)` }} id="connection-container">
                <div ref={containerRef} className={styles.connectionview} style={{ height: `calc(75vh - ${chipsHeight || 0}px)` }} />
                {renderControls(true)}
                <div className={styles.centeredMessage} style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 10, color: '#94A3B8', fontSize: '12px', textAlign: 'center' }}>
                    {renderEmptyMessageText()}
                </div>
            </div>
        );
    }
    ...
    return (
        <>
            <div ref={fullscreenContainerRef} className={isFullscreen ? styles.connectionSearch : styles.containerView} style={{ height: `calc(75vh - ${chipsHeight || 0}px)` }} id="connection-container">
                <div ref={containerRef} className={styles.connectionview} style={{ height: `calc(${isFullscreen ? '103vh' : '75vh'} - ${chipsHeight || 0}px)` }} />
                {renderControls()}
            </div>
...
export default ConnectionViewCriteria;